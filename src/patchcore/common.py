...
class NearestNeighbourScorer(object):
    def __init__(self, n_nearest_neighbours: int, nn_method=FaissNN(False, 4)) -> None:
        ...
        self.n_nearest_neighbours = max(1, n_nearest_neighbours)
        self.nn_method = nn_method
        self.effective_n_neighbours = self.n_nearest_neighbours

        self.imagelevel_nn = lambda query: self.nn_method.run(
            max(1, self.effective_n_neighbours), query
        )
    ...
    def fit(self, detection_features: List[np.ndarray]) -> None:
        ...
        self.effective_n_neighbours = min(
            self.n_nearest_neighbours, len(self.detection_features)
        )
        self.effective_n_neighbours = max(1, self.effective_n_neighbours)
